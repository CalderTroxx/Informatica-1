...
void watch() {
  ...
  for (int i = 3; i >= 0; i--) {
    if (tiempo[i] < 10) {
      lcd.print("0");
      if(i == 0) lcd.print((String) tiempo[i]+ " ");
      else lcd.print((String) tiempo[i] + ":");
    }
    else if(i == 0) lcd.print((String) tiempo[i]+ " ");
    else lcd.print((String) tiempo[i] + ":");
  }
}
...
void botonOne() {
  if (millis() > timeDiference + range) {
    state = !state;
    if(state == 1) TIMSK1 = (1 << OCIE1A);
    if(state == 0) TIMSK1 = (0 << OCIE1A);
